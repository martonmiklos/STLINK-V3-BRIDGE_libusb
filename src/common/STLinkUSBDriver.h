 ...
 */
//******************************************************************************
// ST-Link (v1) enumerates as a mass storage device and must be associated to a
// disk drive letter in order to be handled through this driver.
//
// ST-Link/v2 is handled through its unique
// GUID=DBCE1CD9-A320-4b51-A365-A0C3F3C5FB29, assigned during enumeration with
// .inf file (on Windows). ST-Link/v2-1 and v3 are also handled via their GUID
// (on Windows).
//******************************************************************************
...
#define SS_TCP_CANT_CONNECT                                                    \
    0x2002 // Error when TCP client is not able to connect to the server
...
// Constants defining the type of info to retrieve (STMass_GetDeviceInfo) - Old
// API required for ST-Link/V1
...
#define REQUEST_WRITE                                                          \
    0 // Through EP2OUT or EP1OUT for DBG/TCP interface, through EP6OUT for
      // BRIDGE interface
#define REQUEST_READ_EP3                                                       \
    3 // Through EP3IN or EP2IN for DBG/TCP interface only: Compatibility break
      // if that value was used for reading through EP1
// All other values are for read through EP1IN, but a constant is provided
// anyway for upper layers lisibility
#define REQUEST_READ_EP1                                                       \
    1 // Through EP1IN for DBG interface, through EP6IN for BRIDGE interface
// ----- Define for STLink_DeviceRequestT::InputRequest -----------------------
#define REQUEST_READ_1ST_EPIN                                                  \
    REQUEST_READ_EP1 // read req (cmd+data) through 1st bulk IN EP of the given
                     // interface
#define REQUEST_WRITE_1ST_EPOUT                                                \
    REQUEST_WRITE // write req (cmd+data) through 1st bulk OUT EP of the given
                  // interface
#define REQUEST_READ_DATA_2ND_EPIN                                             \
    REQUEST_READ_EP3 // read req (data only) through 2nd bulk IN EP of the given
...
typedef struct {
    uint8_t CDBLength; // Command header length (CDB on mass storage) - used
                       // only on ST-Link/V1
    uint8_t CDBByte[STLINK_CMD_SIZE_16]; // Command header (CDB on mass storage)
    uint8_t InputRequest;  // 0 (output) or input on ST-Link/V1, REQUEST_WRITE,
                           // REQUEST_READ_EP1 or REQUEST_READ_EP3 on ST-Link/V2
    void *Buffer;          // Data for data stage (if any)
    uint32_t BufferLength; // Size of data stage (0 if no data stage)
    uint8_t SenseLength; // To be initialized: DEFAULT_SENSE_LEN - used only on
                         // ST-Link/V1
    uint8_t Sense[DEFAULT_SENSE_LEN + 2]; // Used only on ST-Link/V1
} TDeviceRequest, *PDeviceRequest; // TDeviceRequest, *PDeviceRequest: legacy
                                   // library name, use STLink_DeviceRequestT

typedef TDeviceRequest STLink_DeviceRequestT;

// Note keep TDeviceRequest before STLink_DeviceRequestT because old library
// only know TDeviceRequest
///////////////////////////////////////////////////////////////////////////////
// Type definitions for new API
///////////////////////////////////////////////////////////////////////////////
// legacy deviceInfo structure before stlink-TCP, use STLink_DeviceInfo2T
// instead
typedef struct {
    char DevPath[MAX_PATH]; // Device pathname returned by
                            // SetupDiGetDeviceInterfaceDetail on ST-Link/V2 Or
                            // disk name like "\\.\A:" on ST-Link/V1
    char EnumUniqueId[SERIAL_NUM_STR_MAX_LEN]; // Unique instance ID from system
                                               // enumeration (equal to device
                                               // Serial number in some cases,
                                               // but not always ...)
    uint16_t
        VendorId; // Vendor  ID from USB device descriptor (system enumeration)
    uint16_t
        ProductId; // Product ID from USB device descriptor (system enumeration)
    uint8_t
        DeviceUsed; // On windows, equal to 1 if device interface was already
                    // opened from externally when enumerating or trying to open
    // NOTE: do not modify the existing fields in the structure. But for any
    // evolution, add a new field at the end in order to keep ascendant
    // compatibility.
...
typedef struct {
    uint32_t StLinkUsbId; ///< ST-LINK-SERVER Device cookie in little endian
                          ///< format, to use in STLINK_TCP_CMD_OPEN_DEVICE
    char EnumUniqueId[SERIAL_NUM_STR_MAX_LEN]; ///< Unique instance ID from
                                               ///< system enumeration (equal to
                                               ///< device Serial number in some
                                               ///< cases, but not always ...)
    uint16_t VendorId;  ///< Vendor  ID from USB device descriptor (system
                        ///< enumeration)
    uint16_t ProductId; ///< Product ID from USB device descriptor (system
                        ///< enumeration)
    uint8_t DeviceUsed; ///< On windows, equal to 1 if device interface was
                        ///< already opened from externally when enumerating or
                        ///< trying to open
    // NOTE: do not modify the existing fields in the structure. But for any
    // evolution, add a new field at the end in order to keep ascendant
    // compatibility.
...
typedef TDeviceInfo2 STLink_DeviceInfo2T; ///< Structure containing information
                                          ///< on connected device connected
                                          ///<  (serial number, device in use,
                                          ///<  USB ProductID, ...).

/// #TEnumStlinkInterface: legacy library name, use #STLink_EnumStlinkInterfaceT
typedef enum {
    STLINK_DBG_INTERFACE =
        0, // Note that the value is also the interface index in the device
           // (important for implementation on Linux)
    STLINK_RW_INTERFACE, // Note that the value is also the interface index in
                         // the device (important for implementation on Linux)
    STLINK_TCP,    // Interface for multi-core debug through TCP client/server
                   // model
    STLINK_BRIDGE, ///< Interface for Bridge
                   // on Linux conversion is done by
                   // STLinkLibUSB_ConvToUsbIfId()
    STLINK_NB_INTERFACES // Keep this last
} TEnumStlinkInterface;

typedef TEnumStlinkInterface
    STLink_EnumStlinkInterfaceT; ///< Enum to choose the STLink interface to be
                                 ///< used
...
#define STLINK_USB_BRIDGE_IF_NB                                                \
    4 // Bridge interface index in the device (important for implementation on
      // Linux) when mass storage interface is present
#define STLINK_USB_BRIDGE_IF_NB_NO_MSD                                         \
    3 // Bridge interface index in the device (important for implementation on
      // Linux) when no mass storage interface
...
// Returned: SS_OK  = SUCCESS, Error otherwise (Error Code)
//******************************************************************************
typedef uint32_t(STD_CALL *pSTMass_Enum_GetDevice)(uint32_t Index,
                                                   PPDevice pDevice);
...
// Returned: SS_OK  = SUCCESS, Error otherwise (Error Code)
//******************************************************************************
typedef uint32_t(STD_CALL *pSTMass_GetDeviceInfo)(PDevice pDevice,
                                                  uint32_t Kind, char *pBuffer,
...
// Returned: SS_OK  = SUCCESS, Error otherwise (Error Code)
//******************************************************************************
typedef uint32_t(STD_CALL *pSTMass_OpenDeviceExclusive)(PDevice pDevice,
                                                        void **pHandle);
...
//             new fields might be added at the end of the structure, without
//             compatibility break.
//
// Returns:
//   SS_OK  = SUCCESS
...
//          TDeviceInfo are significant; the other ones are set to 0.
...
#else
EXPORTED_API uint32_t STD_CALL
STLink_GetDeviceInfo(TEnumStlinkInterface IfId, uint8_t DevIdxInList,
                     TDeviceInfo *pInfo, uint32_t InfoSize);
#endif
...
#else
EXPORTED_API uint32_t STD_CALL
STLink_SendCommand(void *pHandle, PDeviceRequest pRequest, uint32_t DwTimeOut);
#endif
...
typedef uint32_t(STD_CALL *pSTLink_ReenumerateTcp)(TEnumStlinkInterface IfId,
                                                   uint8_t bClearList,
                                                   char *pConnectParams,
                                                   char *pServerCmdLineParams);
#else
EXPORTED_API uint32_t STD_CALL
STLink_ReenumerateTcp(TEnumStlinkInterface IfId, uint8_t bClearList,
...
#else
EXPORTED_API uint32_t STD_CALL
STLink_OpenDeviceTcp(TEnumStlinkInterface IfId, uint32_t DevInfoUsbId,
                     uint8_t bExclusiveAccess);
#endif
...
#else
EXPORTED_API uint32_t STD_CALL
STLink_CloseDeviceTcp(uint32_t StLinkUsbId, uint8_t closeTcp);
#endif
...
#else
EXPORTED_API uint32_t STD_CALL
STLink_SendCommandTcp(uint32_t StLinkUsbId, PDeviceRequest pRequest,
                      uint32_t DwTimeOut);
#endif
...
#else
EXPORTED_API uint32_t STD_CALL
STLink_GetNumOfDeviceClientsTcp(uint32_t StLinkUsbId);
#endif
