...
class StlinkDevice
{
  public:
    ...
    void
    SetOpenModeExclusive(bool bExclusive);
    ...
    /*
     * Getter for m_bStlinkConnected: True if connected to an STLink
     * (PrivOpenStlink() done)
     */
    bool
    GetIsStlinkConnected(void) const
    {
     ...
     */
    uint8_t
    GetMajorVer(void) const
    {
        return m_Version.Major_Ver;
    }
    uint8_t
    GetStm32DbgVer(void) const
    {
        return m_Version.Jtag_Ver;
    }
    uint8_t
    GetStm8DbgVer(void) const
    {
        return m_Version.Swim_Ver;
    }
    uint8_t
    GetMscVcpVer(void) const
    {
        return m_Version.Msc_Ver;
    }
    uint8_t
    GetBridgeVer(void) const
    {
        return m_Version.Bridge_Ver;
    }
    uint16_t
    GetUsbVid(void) const
    {
        return m_Version.VID;
    }
    uint16_t
    GetUsbPid(void) const
    {
        return m_Version.PID;
    }

    // Flag indicating if a session is opened (public for legacy but use getter,
    // may become private)
  ...
  protected:
    STLinkIf_StatusT
    PrivOpenStlink(int StlinkInstId = 0);
    STLinkIf_StatusT
    PrivOpenStlink(const char *pSerialNumber, bool bStrict);
    STLinkIf_StatusT
    PrivCloseStlink(void);
    STLinkIf_StatusT
    PrivGetVersionExt(Stlk_VersionExtT *pVersion);
    STLinkIf_StatusT
    PrivGetTargetVoltage(float *pVoltage);

    STLinkIf_StatusT
    SendRequest(STLink_DeviceRequestT *pDevReq,
