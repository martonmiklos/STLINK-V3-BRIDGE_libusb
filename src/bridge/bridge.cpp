 ...
 *          STLinkInterface to managed USB communication needs to be
 *instanciated and initialized before being used by Brg class. \n Brg is derived
 *of its parent class StlinkDevice.
 ******************************************************************************
 * @attention
 *
 ...
 * @param[in]  StlinkIf  reference to USB STLink Bridge interface:
 ...
 */
Brg_StatusT
Brg::ConvSTLinkIfToBrgStatus(STLinkIf_StatusT IfStat)
{
    Brg_StatusT brgStat;
    switch (IfStat) {
 ...
 */
bool
Brg::IsOldBrgFwVersion(void) const
{
    // Return true if current Bridge FW version is not the last one for given
    // STLink
    if (((m_Version.Major_Ver == 3) &&
         (m_Version.Bridge_Ver < FIRMWARE_BRIDGE_STLINK_V3_LAST_VERSION))) {
        return true;
    } else {
        return false;
    }
 ...
 * This routine use the USB STLink Bridge interface so it is required to have
 * previously called successfully STLinkInterface::LoadStlinkLibrary() to load
 * STLinkUSBDriver library. \n User can call STLinkInterface::EnumDevices() and
 * STLinkInterface::GetDeviceInfo2() to get the list of available USB STLink
 * Bridge interface devices and choose StlinkInstId. If not already done, this
 * routine will enumerate the USB STLink Bridge interface anyway. \n It also
 * checks firmware version. \n StlinkDevice::SetOpenModeExclusive() is used
 * before opening the device to choose the opening mode.
 *
 * @param[in]  StlinkInstId  Instance ID in the list of enumerated STLink
 * devices. from 0 to *pNumDevices-1 (value returned by
 * STLinkInterface::EnumDevices()).
 ...
 * @retval #BRG_OLD_FIRMWARE_WARNING Warning that current Bridge Firmware
 * version is not the last one
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::OpenStlink(int StlinkInstId)
{
    ...
    if (brgStatus == BRG_NO_ERR) {
        ...
        if (IsOldBrgFwVersion() == true) {
            LogTrace(
                "The detected STLink firmware BRIDGE version (V%d.B%d) is "
                "compatible with PC software but is not the most recent one",
...
/**
 * @ingroup DEVICE
 * @brief Same as Brg::OpenStlink(int StlinkInstId) but opening routine based on
 * the serial number of the STLink. \n To be used in case StlinkInstId is no
 * more reliable (devices have been reenumerated between STLink instance
 * selection and opening) or to choose a specific STLink by its serial number.
 *
 * @param[in]  pSerialNumber  STLink serial number (ASCII) (can be retrieved
 * with STLinkInterface::GetDeviceInfo2()).
 ...
 * #BRG_STLINK_SN_NOT_FOUND if the given SerialNumber is not found.\n If bStrict
 * is false and the given serial number is not found and one (and only one)
 *                       STLink is found, the command will attempt to open the
 * STLink even if the serial number does not match.
 * @warning #BRG_OLD_FIRMWARE_WARNING is not a fatal error.
 * @return Brg::ConvSTLinkIfToBrgStatus(STLinkInterface::EnumDevices()) errors
 ...
 */
Brg_StatusT
Brg::OpenStlink(const char *pSerialNumber, bool bStrict)
{
    STLinkIf_StatusT ifStatus = STLINKIF_NO_ERR;
    Brg_StatusT brgStatus;
...
/**
 * @ingroup DEVICE
 * @brief Close STLink USB communication with the device instance that was
 * opened by Brg::OpenStlink().
 *
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::CloseStlink(void)
{
...
/**
 * @ingroup DEVICE
 * @brief This routine closes given or all Bridge communication(s), need to call
 * again Init function after if new bridge communication is required.
 * @param[in]  BrgCom Communication(s) to be closed: One of #COM_I2C, #COM_SPI,
 * #COM_CAN, #COM_GPIO \n or #COM_UNDEF_ALL for all.
 *
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::CloseBridge(uint8_t BrgCom)
{
...
/**
 * @ingroup DEVICE
 * @brief This routine gets USB VID and PID, and firmware version of the STLink
 * Bridge device.
 ...
 */
Brg_StatusT
Brg::ST_GetVersionExt(Stlk_VersionExtT *pVersion)
{
    STLinkIf_StatusT ifStatus = STLINKIF_NO_ERR;
    ifStatus = StlinkDevice::PrivGetVersionExt(pVersion);
    return ConvSTLinkIfToBrgStatus(ifStatus);
}
/* Send a command over the USB, wait for answer, and analyze the returned status
 * if pStatus!=NULL UsbTimeoutMs if 0 use default (5s) else use UsbTimeoutMs.
 * Requires the STLink to be already connected (m_bStlinkConnected==true)
 */
Brg_StatusT
Brg::SendRequestAndAnalyzeStatus(STLink_DeviceRequestT *pDevReq,
                                 ...
                                 const uint16_t UsbTimeoutMs)
{
    ...
    if (brgStat == BRG_TARGET_CMD_ERR) {
        ...
        LogTrace(
            "BRIDGE Error (0x%hx) after BRIDGE cmd %02hX %02hX %02hX %02hX "
            "%02hX %02hX %02hX %02hX %02hX %02hX",
            // printf("Error (0x%hx) after target cmd %02hX %02hX %02hX %02hX
            // %02hX %02hX %02hX %02hX %02hX %02hX",
            (unsigned short)*pStatus, (unsigned short)pDevReq->CDBByte[0],
...
/*
 * Analyze the STLink returned status if pStatus!=NULL and convert it to Bridge
 * status
 */
Brg_StatusT
Brg::AnalyzeStatus(const uint16_t *pStatus)
{
    if (pStatus != NULL) {
        if (*pStatus != STLINK_BRIDGE_OK) {
            ...
            if (*pStatus == STLINK_BRIDGE_INIT_NOT_DONE) {
                LogTrace("This BRIDGE command requires the com to be "
                         "initialized: call "
                         "Init function");
                return BRG_COM_INIT_NOT_DONE;
            }
            if (*pStatus == STLINK_BRIDGE_ABORT_TRANS) {
                LogTrace("BRIDGE Incorrect command order in partial (I2C) "
                         "transaction, "
            ...
            if (*pStatus == STLINK_BRIDGE_CMD_BUSY) {
                LogTrace("BRIDGE Command busy (only GET_RWCMD_STATUS allowed "
                         "in this "
                         "state)");
                return BRG_CMD_BUSY;
            }
...
/**
 * @ingroup BRIDGE
 * @brief This routine gets some current frequencies, useful to choose bridge
 * initialization parameters.
 * @warning: Frequency parameters are valid while STLink frequency is not
 * changed on debug interface by using STLINK_SWITCH_STLINK_FREQ.
 ...
 */
Brg_StatusT
Brg::GetClk(uint8_t BrgCom, uint32_t *pBrgInputClk, uint32_t *pStlHClk)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    *pBrgInputClk = (uint32_t)answer[4] | (uint32_t)answer[5] << 8 |
                    (uint32_t)answer[6] << 16 | (uint32_t)answer[7] << 24;
    *pStlHClk = (uint32_t)answer[8] | (uint32_t)answer[9] << 8 |
                (uint32_t)answer[10] << 16 | (uint32_t)answer[11] << 24;
 ...
 * Refer to STLink firmware upgrade release note (stsw-link007) for supported
 * init configurations.\n Call Brg::GetSPIbaudratePrescal() to get baudrate
 * parameter according to required SPI output frequency.
 * @warning: Baudrate prescaler parameter is valid while STLink frequency is not
 * changed on debug interface by using STLINK_SWITCH_STLINK_FREQ.
 * @param[in]  pInitParams SPI initialization parameters see #Brg_SpiInitT and
 * \ref LIMITATION
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 ...
 */
Brg_StatusT
Brg::InitSPI(const Brg_SpiInitT *pInitParams)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint16_t status;
    ...
    pRq->CDBByte[3] = ((uint8_t)pInitParams->Mode & 0x01) |
                      ((((uint8_t)pInitParams->Cpha) << 1) & 0x02) |
                      ((((uint8_t)pInitParams->Cpol) << 2) & 0x04) |
                      ((((uint8_t)pInitParams->FirstBit) << 3) & 0x08) |
                      ((((uint8_t)pInitParams->FrameFormat) << 4) & 0x10);
    // Data size
    pRq->CDBByte[4] = (uint8_t)(pInitParams->DataSize);
    // Slave Select management
    pRq->CDBByte[5] = ((uint8_t)pInitParams->Nss & 0x1) |
                      ((((uint8_t)pInitParams->NssPulse) << 1) & 0x2);
    // BaudRate Prescaler
...
/**
 * @ingroup SPI
 * @brief This routine gets the SPI baudrate prescaler needed for Brg::InitSPI()
 * according to required SPI CLK.
 * @param[in]  ReqSpiFreqKHz Required SPI CLK (SCK) in KHz
 * @param[out] pBaudrate Baudrate parameter to use (in Brg_SpiInitT
 * Brg::InitSPI())
 * @param[out] pFinalSpiFreqKHz Final SPI CLK (SCK) in KHz corresponding to
 * returned baudrate (may be less than ReqSpiFreqKHz)
 * @warning: Baudrate parameter is valid while STLink frequency is not changed
 * on debug interface by using STLINK_SWITCH_STLINK_FREQ.\n
 *           #BRG_COM_FREQ_MODIFIED is not a fatal error.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_FREQ_MODIFIED If ReqSpiFreqKHz is different from
 * pFinalSpiFreqKHz
 * @retval #BRG_PARAM_ERR In case of wrong parameter
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::GetSPIbaudratePrescal(uint32_t ReqSpiFreqKHz, Brg_SpiBaudrateT *pBaudrate,
                           uint32_t *pFinalSpiFreqKHz)
{
    Brg_StatusT brgStat;
    ...
    if ((ReqSpiFreqKHz == 0) || (pBaudrate == NULL) ||
        (pFinalSpiFreqKHz == NULL)) {
        return BRG_PARAM_ERR;
    }
    if (m_bStlinkConnected == false) {
        // The function should be called at least after OpenStlink
...
/**
 * @ingroup SPI
 * @brief This routine allows to manage SPI NSS (nCS) pin when SPI has been
 * initialized with #SPI_NSS_SOFT.
 ...
 */
Brg_StatusT
Brg::SetSPIpinCS(Brg_SpiNssLevelT NssLevel)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint16_t status;
...
/**
 * @ingroup SPI
 * @brief This routine allows to receive bytes on the SPI interface, in the mode
 * initialized by Brg::InitSPI().\n In #SPI_DIRECTION_2LINES_FULLDUPLEX dummy
 * data are sent while receiving.
 * @param[out] pBuffer Pointer on data buffer filled with read data.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 * size)
 * @param[out] pSizeRead If not NULL and in case of error, pSizeRead returns the
 * number of bytes received before the error.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If SPI is not initialized
 ...
 */
Brg_StatusT
Brg::ReadSPI(uint8_t *pBuffer, uint16_t SizeInBytes, uint16_t *pSizeRead)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    if (brgStat == BRG_NO_ERR) { // pErrorInfo currently unused
        brgStat = GetLastReadWriteStatus(pSizeRead, NULL);
    }

    if (brgStat != BRG_NO_ERR) {
        LogTrace("SPI Error (%d) in ReadSPI (%d bytes)", (int)brgStat,
                 (int)SizeInBytes);
        if (pSizeRead != NULL) {
...
/**
 * @ingroup SPI
 * @brief This routine allows to transmit bytes on the SPI interface, in the
 * mode initialized by Brg::InitSPI().
 * @param[in]  pBuffer Pointer on data buffer with data to be sent.
 * @param[in]  SizeInBytes Data size to be sent in bytes (min 1, max data buffer
 * size)
 * @param[out] pSizeWritten If not NULL and in case of error, pSizeWritten
 * returns the number of bytes transmitted before the error.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If SPI is not initialized
 ...
 */
Brg_StatusT
Brg::WriteSPI(const uint8_t *pBuffer, uint16_t SizeInBytes,
              uint16_t *pSizeWritten)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    if (brgStat != BRG_NO_ERR) {
        LogTrace("SPI Error (%d) in WriteSPI (%d bytes)", (int)brgStat,
                 (int)SizeInBytes);
        if (pSizeWritten != NULL) {
            LogTrace("SPI Only %d bytes written without error",
                     (int)*pSizeWritten);
        }
    }
    return brgStat;
 ...
 * Refer to STLink firmware upgrade release note (stsw-link007) for supported
 * init configurations.\n Call Brg::GetI2cTiming() to get timing parameter
 * according to required I2C speed and time configuration parameters.
 * @param[in]  pInitParams I2C initialization parameters see #Brg_I2cInitT and
 * \ref LIMITATION
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_PARAM_ERR Null pointer parameter
 ...
 */
Brg_StatusT
Brg::InitI2C(const Brg_I2cInitT *pInitParams)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint16_t status;
    ...
    // Filters : analog and digital filters config : Bit3-0: DNF (<=15)  Bit7:
    // analog filter 0 OFF, 1 ON
    //  Bit6-4: 0 (reserved)
    if (pInitParams->DigitalFilterEn == I2C_FILTER_DISABLE) {
        ...
        if (pInitParams->Dnf <= 15) {
            pRq->CDBByte[9] =
                ((uint8_t)pInitParams->Dnf & 0x0F) |
                ((((uint8_t)pInitParams->AnFilterEn) << 7) & 0x80);
        } else {
            delete pRq;
            return BRG_PARAM_ERR;
        }
...
/**
 * @ingroup I2C
 * @brief This routines can be used to get timing parameter (pTimingReg)
 * required by Brg::InitI2C().\n It set timing parameter according to required
 * I2C speed, mode, rise/fall time and filter configuration.
 * @param[in]  I2CSpeedMode  #I2C_STANDARD, #I2C_FAST, #I2C_FAST_PLUS
 * @param[in]  SpeedFrequency  In KHz, 1-100KHz (STANDARD), 1-400KHz (FAST),
 * 1-1000KHz (FAST PLUS)
 * @param[in]  DNFn  0 (no digital filter) up to 15, noise digital filter (delay
 * = DNFn/SpeedFrequency)
 * @param[in]  RiseTime In ns, 0-1000ns (STANDARD), 0-300ns (FAST), 0-120ns
 * (FAST PLUS)
 * @param[in]  FallTime In ns, 0-300ns (STANDARD), 0-300ns (FAST), 0-120ns (FAST
 * PLUS)
 * @param[in]  bAF  Use true for Analog Filter ON or false for Analog Filter OFF
 * @param[out] pTimingReg  Filled with timing parameter required by
 * Brg::InitI2C().
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_PARAM_ERR Null pointer parameter
 ...
 */
Brg_StatusT
Brg::GetI2cTiming(I2cModeT I2CSpeedMode, int SpeedFrequency, int DNFn,
                  int RiseTime, int FallTime, bool bAF, uint32_t *pTimingReg)
{
    ...
    *pTimingReg = 0; // Default if error
    if ((SpeedFrequency < 1) || (RiseTime < 0) || (FallTime < 0) ||
        (DNFn > 15) || (DNFn < 0) ||
        ((I2CSpeedMode == I2C_STANDARD) &&
         ((SpeedFrequency > 100) || (RiseTime > 1000) || (FallTime > 300))) ||
        ((I2CSpeedMode == I2C_FAST) &&
         ((SpeedFrequency > 400) || (RiseTime > 300) || (FallTime > 300))) ||
        ((I2CSpeedMode == I2C_FAST_PLUS) &&
         ((SpeedFrequency > 1000) || (RiseTime > 120) || (FallTime > 120)))) {
        return BRG_PARAM_ERR;
    }
    ...
    if (brgStatus == BRG_NO_ERR) {
        ...
        brgStatus =
            CalculateI2cTimingReg(I2CSpeedMode, SpeedFrequency, clockSource,
                                  DNFn, RiseTime, FallTime, bAF, pTimingReg);
    }
 ...
 */
Brg_StatusT
Brg::CalculateI2cTimingReg(I2cModeT I2CSpeedMode, int SpeedFrequency,
                           double ClockSource, int DNFn, int RiseTime,
...
#define MAX_I2C_MODEL_NB 40
    ...
    Brg_I2cModelT *pPrescSDLDELSCLDELpossComb =
        new Brg_I2cModelT[MAX_I2C_MODEL_NB]; // possible combination table
    ...
    tmpMinSDADEL =
        (double)((double)fallTimeCalc - (50 / (double)pow((double)10, 9)) -
                 (double)((DNFn + 3) * clkPeriodI2C));
    tmpMaxSDADEL = (double)((double)pTHdDatMax[idx] - riseTimeCalc -
                            (1 * (260 / (double)pow((double)10, 9))) -
    ...
    for (int i1 = 0; i1 < PRESC_LENGTH; i1++) {
        ...
        for (int i3 = 0; i3 < SCLDEL_LENGTH; i3++) {
            tmpSCLDEL = (double)((pSCLDEL[i3] + 1) *
                                 (double)((pPRESC[i1] + 1) * clkPeriodI2C));

            for (int i2 = 0; i2 < SCLDEL_LENGTH; i2++) {
                tmpSDADEL = (double)(pSDADEL[i2] *
                                     (double)((pPRESC[i1] + 1) * clkPeriodI2C));
                if ((tmpSDADEL >= tmpMinSDADEL) &&
                    (tmpSDADEL <= tmpMaxSDADEL) &&
                    (tmpSCLDEL >= tmpMinSCLDEL)) {
                    if (pPRESC[i1] != tmpPRESC) {
                        ...
                        // else MAX_I2C_MODEL_NB should be incremented or use
                        // dynamic memory
                        tabIndex++;
                    }
                }
        ...
        for (int i3 = nbValid - 1; i3 >= 0; i3--) {
            for (int i1 = 0; i1 < SCLL_LENGTH; i1++) {
                for (int i2 = 0; i2 < SCLH_LENGTH; i2++) {
                    double presc =
                        (double)(pPRESCvalid[i3] + 1) * (double)clkPeriodI2C;
                    ...
                    double tScl = (double)tSclLow + (double)tSclHigh +
                                  (double)riseTimeCalc + (double)fallTimeCalc;
                    double speed = 1 / (double)tScl;

                    if ((speed >= clkMin) && (speed <= clkMax) &&
                        (tSclLow >= pTLowMin[idx]) &&
                        (tSclHigh >= pTHighMin[idx]) &&
                        (clkPeriodI2C < ((tSclLow - delayFilter) / 4)) &&
                        (clkPeriodI2C < tSclHigh)) {
                        double errorTmp = (double)(speed - targetFreqI2C) /
                                          (double)(targetFreqI2C);
                        if (errorTmp < 0) {
                    ...
                    if ((s.resultat <= errorTarget) &&
                        (pPRESCvalid[i3] <= prescR)) {
            ...
            for (int i3 = 0; i3 < 16; i3++) {
                for (int j = 0; j < prescSDLDELSCLDELpossCombSize; j++) {
                    if ((pPrescSDLDELSCLDELpossComb[j].resultat == 1) &&
                        (pPrescSDLDELSCLDELpossComb[j].prescaler ==
                         pPRESCvalid[i3Sel]) &&
    ...
    *pTimingReg = 0;
    if (solution == 1) {
        *pTimingReg =
            (uint32_t)((uint32_t)pPRESCvalid[i3Sel]
                           << 28 | // Bits 31:28 PRESC[3:0]: Timing prescaler
                       (uint32_t)scldel
                           << 20 | // Bits 27:24 Reserved, must be kept at reset
                                   // value. Bits 23:20 SCLDEL[3:0]: Data setup
                                   // time
                       (uint32_t)sdadel
                           << 16 | // Bits 19:16 SDADEL[3:0]: Data hold time
                       (uint32_t)i2Sel << 8 | // Bits 15:8 SCLH[7:0]: SCL high
                                              // period (master mode)
                       (uint32_t)i1Sel); // Bits 7:0 SCLL[7:0]: SCL low period
                                         // (master mode)
...
/**
 * @ingroup I2C
 * @brief This routine allows to receive bytes on the I2C interface, in the mode
 * initialized by Brg::InitI2C().\n In master mode I2C, the size bytes are
 * received in one I2C transaction ((re)start- addr- data -stop) even if data
 * may be split into several USB packets.
 ...
 * @retval #BRG_COM_CMD_ORDER_ERR If low level I2C function call order is not
 * consitent (Start, Cont, Stop)
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::ReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes,
             uint16_t *pSizeRead)
{
...
/**
 * @ingroup I2C
 * @brief Same as Brg::ReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t
 * SizeInBytes, uint16_t *pSizeRead) except for Addr parameter
 ...
 */
Brg_StatusT
Brg::ReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode,
 ...
 */
Brg_StatusT
Brg::ReadI2Ccmd(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes,
                Brg_I2cRWTransfer RwTransType, uint16_t *pSizeRead,
                uint32_t *pErrorInfo)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    if (brgStat != BRG_NO_ERR) {
        LogTrace("I2C Error (%d) in ReadI2C (%d bytes)", (int)brgStat,
                 (int)SizeInBytes);
        if (pSizeRead != NULL) {
...
/**
 * @ingroup I2C
 * @brief This low level routine allows to split I2C transaction and to receive
 * bytes on the I2C interface, in the mode initialized by Brg::InitI2C().\n For
 * a full I2C transaction use Brg::ReadI2C() instead.\n Master mode I2C
 * transaction is splitted: only (re)start, Addr are sent and data are received
 * (no stop sent).
 * @warning After Brg::StartReadI2C() it is mandatory to call Brg::StopReadI2C()
 * (optionally Brg::ContReadI2C()).
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 ...
 */
Brg_StatusT
Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes,
                  uint16_t *pSizeRead)
{
    Brg_StatusT status;
    m_slaveAddrPartialI2cTrans = Addr;
    status = ReadI2Ccmd(pBuffer, Addr, SizeInBytes, I2C_START_RW_TRANS,
                        pSizeRead, NULL);
    return status;
}
/**
 * @ingroup I2C
 * @brief Same as Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t
 * SizeInBytes, uint16_t *pSizeRead) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT
Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode,
                  uint16_t SizeInBytes, uint16_t *pSizeRead)
{
    uint16_t slaveAddr = Addr; // default bit15=0 7b
    if (AddrMode == I2C_ADDR_10BIT) {
 ...
 * bytes on the I2C interface, in the mode initialized by Brg::InitI2C().\n For
 * a full I2C transaction use Brg::ReadI2C() instead.\n Master mode I2C
 * transaction is splitted: only data are received (no start-address or stop
 * sent).
 * @warning Before Brg::ContReadI2C() it is mandatory to call
 * Brg::StartReadI2C().\n After Brg::ContReadI2C() it is mandatory to call
 * Brg::StopReadI2C().
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 * size)
 ...
 */
Brg_StatusT
Brg::ContReadI2C(uint8_t *pBuffer, uint16_t SizeInBytes, uint16_t *pSizeRead)
{
    Brg_StatusT status;
    status = ReadI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes,
                        I2C_CONT_RW_TRANS, pSizeRead, NULL);
    return status;
}
/**
 * @ingroup I2C
 ...
 * bytes on the I2C interface, in the mode initialized by Brg::InitI2C().\n For
 * a full I2C transaction use Brg::ReadI2C() instead.\n Master mode I2C
 * transaction is splitted: only data are received and stop sent (no
 * start-address sent).
 * @warning Before Brg::StopReadI2C() it is mandatory to call
 * Brg::StartReadI2C() (optionally Brg::ContReadI2C()).
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 * size)
 * @param[out] pSizeRead If not NULL and in case of error, pSizeRead returns the
 ...
 */
Brg_StatusT
Brg::StopReadI2C(uint8_t *pBuffer, uint16_t SizeInBytes, uint16_t *pSizeRead)
{
    Brg_StatusT status;
    status = ReadI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes,
                        I2C_STOP_RW_TRANS, pSizeRead, NULL);
    return status;
}
/**
 * @ingroup I2C
 * @brief This routine allows to receive bytes on the I2C interface without
 * blocking the USB for I2C transaction completion.\n Main use case if for I2C
 * transaction requiring timeout higher than 200ms to complete. Difference with
 * Brg::ReadI2C() are:
 * - Command answer is a status not the data.
 * - I2C transaction at target side is not completed when receiving the status
 * answer.
 * - BUSY is returned in the status while I2C transaction at target side is not
 * completed, in BUSY state only Brg:GetLastReadWriteStatus() command can be
 * sent to the firmware.
 * - Once status is no more BUSY, data can be retrieved using
 * Brg:GetReadDataI2C().
 * - Data size limited to 512 bytes.
 * @warning If other commands than STLINK_BRIDGE_GET_RWCMD_STATUS are sent to
 * the STLink firmware while it is in BUSY state this can break the USB
 * communication.
 *
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 ...
 * @param[out] pSizeRead If not NULL and in case of error, pSizeRead returns the
 * number of bytes received before the error.
 * @param[in]  CmdTimeoutMs 0: #DEFAULT_CMD_TIMEOUT (~200ms), val>0: timeout to
 * wait for the command completion, value in ms (must be a multiple of 200ms, up
 * to 50s). While command is not completed or CmdTimeoutMs not elapsed STLink
 * firmware is BUSY.
 *
 * @retval #BRG_CMD_BUSY If I2C transaction is not completed, use
 * Brg:GetLastReadWriteStatus() until no more BUSY
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::ReadNoWaitI2C(uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeRead,
                   uint16_t CmdTimeoutMs)
{
    ...
    if (CmdTimeoutMs != DEFAULT_CMD_TIMEOUT) {
        if (CmdTimeoutMs > 50000) { // trunk to 50s (max uint8_t 0xFF = 256)
            ...
            targetCmdTimeout =
                (uint8_t)((CmdTimeoutMs + 199) /
                          200); // CmdTimeoutMs = targetCmdTimeout*200ms
    ...
    if (brgStat ==
        BRG_NO_ERR) // answer is same format as GetLastReadWriteStatus()
    {
    ...
    if (brgStat == BRG_CMD_BUSY) {
        LogTrace("I2C (Busy) (%d) in ReadNoWaitI2C (%d bytes)", (int)brgStat,
                 (int)SizeInBytes);
    } else if (brgStat != BRG_NO_ERR) {
        LogTrace("I2C Error (%d) in ReadNoWaitI2C (%d bytes)", (int)brgStat,
                 (int)SizeInBytes);
        if (pSizeRead != NULL) {
            LogTrace("I2C Only %d bytes read without error", (int)*pSizeRead);
        }
...
/**
 * @ingroup I2C
 * @brief Same as Brg::ReadNoWaitI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t
 * SizeInBytes, uint16_t *pSizeRead) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT
Brg::ReadNoWaitI2C(uint16_t Addr, Brg_I2cAddrModeT AddrMode,
                   uint16_t SizeInBytes, uint16_t *pSizeRead,
                   uint16_t CmdTimeoutMs)
{
    uint16_t slaveAddr = Addr; // Default bit15=0 7b
...
/**
 * @ingroup I2C
 * @brief This routine allows to get the data read on I2C with
 * Brg::ReadNoWaitI2C() once status is no more BUSY. Status is retrieved using
 * Brg::GetLastReadWriteStatus() (#BRG_CMD_BUSY is answered in BUSY state).
 * @warning If other commands than STLINK_BRIDGE_GET_RWCMD_STATUS are sent to
 * the STLink firmware while it is in BUSY state this can break the USB
 * communication.
 *
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes must be same as the one
 * used in Brg::ReadNoWaitI2C() (min 1, max data buffer size).
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
 ...
 */
Brg_StatusT
Brg::GetReadDataI2C(uint8_t *pBuffer, uint16_t SizeInBytes)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    // send get status to be sure we are not in busy must be done outside this
    // command using: brgStat = GetLastReadWriteStatus(NULL, NULL)
    brgStat = BRG_NO_ERR;
...
/**
 * @ingroup I2C
 * @brief This routine allows to transmit bytes on the I2C interface, in the
 * mode initialized by Brg::InitI2C().\n In master mode I2C the size bytes are
 * transmitted in one I2C transaction ((re)start- addr- data -stop) even if data
 * may be split into several USB packets.
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 ...
 */
Brg_StatusT
Brg::WriteI2C(const uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes,
              uint16_t *pSizeWritten)
{
    return WriteI2Ccmd(pBuffer, Addr, SizeInBytes, I2C_FULL_RW_TRANS,
                       pSizeWritten, NULL);
}
/**
 * @ingroup I2C
 * @brief Same as WriteI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t Size,
 * uint16_t *pSizeWritten) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT
Brg::WriteI2C(const uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode,
 ...
 */
Brg_StatusT
Brg::WriteI2Ccmd(const uint8_t *pBuffer, uint16_t Addr, uint16_t Size,
                 Brg_I2cRWTransfer RwTransType, uint16_t *pSizeWritten,
                 uint32_t *pErrorInfo)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    if (brgStat != BRG_NO_ERR) {
        LogTrace("I2C Error (%d) in WriteI2C (%d bytes)", (int)brgStat,
                 (int)Size);
        if (pSizeWritten != NULL) {
            LogTrace("I2C Only %d bytes written without error",
                     (int)*pSizeWritten);
        }
    }
    return brgStat;
...
/**
 * @ingroup I2C
 * @brief This low level routine allows to split I2C transaction and to send
 * bytes on the I2C interface, in the mode initialized by Brg::InitI2C().\n For
 * a full I2C transaction use Brg::WriteI2C() instead.\n Master mode I2C
 * transaction is splitted: only (re)start- addr- data are sent (no stop).
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 ...
 */
Brg_StatusT
Brg::StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes,
                   uint16_t *pSizeWritten)
{
    Brg_StatusT status;
    m_slaveAddrPartialI2cTrans = Addr;
    status = WriteI2Ccmd(pBuffer, Addr, SizeInBytes, I2C_START_RW_TRANS,
                         pSizeWritten, NULL);
    return status;
}
/**
 * @ingroup I2C
 * @brief Same as Brg::StartWriteI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t
 * SizeInBytes, uint16_t *pSizeWritten) except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 ...
 */
Brg_StatusT
Brg::StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr,
                   Brg_I2cAddrModeT AddrMode, uint16_t SizeInBytes,
                   uint16_t *pSizeWritten)
{
    uint16_t slaveAddr = Addr; // default bit15=0 7b
    if (AddrMode == I2C_ADDR_10BIT) {
 ...
 * a full I2C transaction use Brg::WriteI2C() instead.\n Master mode I2C
 * transaction is splitted: only (re)start- addr- data are sent (no stop).
 * @warning Before Brg::ContWriteI2C() it is mandatory to call
 * Brg::StartWriteI2C().\n After Brg::ContWriteI2C() it is mandatory to call
 * Brg::StopWriteI2C().
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 * size).
 ...
 */
Brg_StatusT
Brg::ContWriteI2C(const uint8_t *pBuffer, uint16_t SizeInBytes,
                  uint16_t *pSizeWritten)
{
    Brg_StatusT status;
    status = WriteI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes,
                         I2C_CONT_RW_TRANS, pSizeWritten, NULL);
    return status;
}
/**
 * @ingroup I2C
 ...
 * bytes on the I2C interface, in the mode initialized by Brg::InitI2C().\n For
 * a full I2C transaction use Brg::WriteI2C() instead.\n Master mode I2C
 * transaction is splitted: only data and stop are sent (no start-address).
 * @warning Before Brg::StopWriteI2C() it is mandatory to call
 * Brg::StartWriteI2C() (optionally Brg::ContWriteI2C()).
 * @param[in]  pBuffer Pointer on data buffer with data to be read.
 * @param[in]  SizeInBytes Data size to be read in bytes (min 1, max data buffer
 * size).
 * @param[out] pSizeWritten If not NULL and in case of error, pSizeWritten
 ...
 */
Brg_StatusT
Brg::StopWriteI2C(const uint8_t *pBuffer, uint16_t SizeInBytes,
                  uint16_t *pSizeWritten)
{
    Brg_StatusT status;
    status = WriteI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes,
                         I2C_STOP_RW_TRANS, pSizeWritten, NULL);
    return status;
}
/**
 * @ingroup CAN
 ...
 * Refer to STLink firmware upgrade release note (stsw-link007) for supported
 * init configurations.\n Call Brg::GetCANbaudratePrescal() to get prescaler
 * parameter according to required CAN bitrate.
 * @warning Prescaler parameter is valid while STLink frequency is not changed
 * on debug interface by using STLINK_SWITCH_STLINK_FREQ.
 * @param[in]  pInitParams CAN initialization parameters see #Brg_CanInitT and
 * \ref LIMITATION
 * @param[in]  InitType  #BRG_INIT_FULL for normal case (CLK, reset, Filter
 ...
 */
Brg_StatusT
Brg::InitCAN(const Brg_CanInitT *pInitParams, Brg_InitTypeT InitType)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint16_t status;
    ...
    if ((pBitTimeConf->PropSegInTq < 1) ||
        (pBitTimeConf->PropSegInTq > 8) || // PROP_SEG = 1 to 8 time quantum
        (pBitTimeConf->PhaseSeg1InTq < 1) ||
        (pBitTimeConf->PhaseSeg1InTq > 8) || // PHASE_SEG1 = 1 to 8 time quantum
        (pBitTimeConf->PhaseSeg2InTq < 1) ||
        (pBitTimeConf->PhaseSeg2InTq > 8) || // PHASE_SEG2 = 1 to 8 time quantum
        (pBitTimeConf->SjwInTq < 1) ||
        (pBitTimeConf->SjwInTq >
         4)) { // Synchro Jump Width = 1 to 4 time quantum
        return BRG_PARAM_ERR;
    }
    // Check prescaler
    ...
    pRq->CDBByte[3] =
        (uint8_t)(((pBitTimeConf->PhaseSeg1InTq - 1) & 0x07) |
                  (((pBitTimeConf->PropSegInTq - 1) << 3) & 0x38));
    // Bit Time segment2 BS2 (PHASE_SEG2) and SJW (Synchronisation Jump Width)
    // CAN_1_time_quantum = 0 Bit 2-0:  segment2 (BS2) Bit 4-3: SJW
    pRq->CDBByte[4] = (uint8_t)(((pBitTimeConf->PhaseSeg2InTq - 1) & 0x07) |
                                (((pBitTimeConf->SjwInTq - 1) << 3) & 0x18));
    // Configuration
    // Bit0 reserved: 0
    ...
    // Bit3 Nart : Enable (1) or disable (0) the no-automatic retransmission
    // mode Bit4 Rflm : Enable (1) or disable (0) the Receive FIFO Locked mode
...
/**
 * @ingroup CAN
 * @brief This routine gets the CAN  prescaler needed for Brg::InitCAN()
 * according to required CAN bitrate.\n Refer to STLink firmware upgrade release
 * note (stsw-link007) for supported init configurations.
 * @warning pPrescal parameter is valid while STLink frequency is not changed on
 * debug interface by using STLINK_SWITCH_STLINK_FREQ.
 * @param[in]  pBitTimeConf  CAN Bit Time configuration that will also be used
 * in Brg::CAN_Init().
 * @param[in]  ReqBaudrate  Requested baudRate (max 1000000 1Mbps).
 * @param[out] pPrescal  Prescaler to be used in Brg::InitCAN() parameters for
 * the given bitrate and bit Time configuration.\n If found prescaler is <1
 * baudrate or N need to be decreased, if if found prescaler is >1024 baudrate
 * or N need to be increased.\n Tq = time quanta \n BaudRate = 1/NominalBitTime
 * \n <table> <tr><td colspan="4">Nominal bit Time <tr><td>sync_seg <td>prop_seg
 * <td>phase_seg1    <td>phase_seg2 <tr><td>1xTq          <td>1 to 8 xTq <td>1
 * to 8 xTq    <td>1 to 8 xTq <tr><td>              <td colspan="2">Tbs1
 * <td>Tbs2
 *    </table>
 ...
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_PARAM_ERR If null pointer parameter or incorrect
 * #Brg_CanBitTimeConfT
 * @retval #BRG_COM_FREQ_MODIFIED If ReqBaudrate is different from
 * pFinalBaudrate
 * @retval #BRG_COM_FREQ_NOT_SUPPORTED If found prescaler is not in the correct
 * range (1-1024)
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::GetCANbaudratePrescal(const Brg_CanBitTimeConfT *pBitTimeConf,
                           uint32_t ReqBaudrate, uint32_t *pPrescal,
                           uint32_t *pFinalBaudrate)
{
    Brg_StatusT brgStat;
    ...
    if ((pBitTimeConf == NULL) || (pPrescal == NULL) ||
        (pFinalBaudrate == NULL)) {
        return BRG_PARAM_ERR;
    }
    ...
    if (brgStat == BRG_NO_ERR) {
        /* Bit timing
          The bit timing logic monitors the serial bus-line and performs
          sampling and adjustment of the sample point by synchronizing on the
          start-bit edge and resynchronizing on the following edges. Its
          operation may be explained simply by splitting nominal bit time into
          three segments as follows:
          - Synchronization segment (SYNC_SEG): a bit change is expected to
          occur within this time segment. It has a fixed length of one time
          quantum (1 x tq).
          - Bit segment 1 (BS1): defines the location of the sample point. It
          ...
          - Bit segment 2 (BS2): defines the location of the transmit point. It
          ...
          shortened to compensate for negative phase drifts. The
          resynchronization Jump Width (SJW) defines an upper bound to the
          amount of lengthening or shortening of the bit segments. It is
          programmable between 1 and 4 time quanta Tq = time quanta BaudRate =
          1/NominalBitTime
          <--------------- Nominal bit Time --------------------------------->
          ...
          <-- 1xTq --><- 1 to 8 x Tq -><-- 1 to 8 x Tq  --><-- 1 to 8 x Tq -->
                                  <-------- Tbs1 ---------------------><-----
          Tbs2
          ------> NominalBitTime = Tsync_seg + Tprop_seg + Tphase_seg1 +
          Tphase_seg2 = 1xTq + Tbs1 +Tbs2 = NxTq Tq = Prescaler x TcanClk
          Prescaler = 1/ (N x BaudRate x TcanClk) = FcanClk / (N x BaudRate)
        */
        // Calculate Prescaler = 1/ (N x BaudRate x TcanClk) = FcanClk / (N x
        // BaudRate) N = 1 + PROP_SEG + PHASE_SEG1 + PHASE_SEG2
        n = 1 + pBitTimeConf->PropSegInTq + pBitTimeConf->PhaseSeg1InTq +
            pBitTimeConf->PhaseSeg2InTq;
        ...
        *pFinalBaudrate = canInputClkKHz * 1000 / (n * calcPrescal);
        ...
        if ((calcPrescal < 1) || (calcPrescal > 1024)) {
            ...
            brgStat = BRG_COM_FREQ_MODIFIED; // ReqBaudrate is different from
                                             // pFinalBaudrate
        }
    }
 ...
 */
Brg_StatusT
Brg::FormatFilter32bitCAN(const Brg_FilterBitsT *pInConf, uint8_t *pOutConf)
{
    ...
    pOutConf[0] |=
        (uint8_t)(((pInConf->ID >> 11) << 3) & 0xF8);      // [7:3]= Id[15:11]
 ...
 */
Brg_StatusT
Brg::FormatFilter16bitCAN(const Brg_FilterBitsT *pInConf, uint8_t *pOutConf)
{
    // filterId/Mask format default 0: Id=0, CAN_ID_STANDARD, CAN_DATA_FRAME
    Brg_StatusT brgStat = BRG_NO_ERR;
    // filterId/Mask format default 0: Id=0, CAN_ID_STANDARD, CAN_DATA_FRAME
...
/**
 * @ingroup CAN
 * @brief This routine initializes the CAN Filters according to init
 * parameters.\n 14 filters can be configured.
 * @param[in]  pInitParams CAN filter configuration see #Brg_CanFilterConfT for
 * details.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 ...
 */
Brg_StatusT
Brg::InitFilterCAN(const Brg_CanFilterConfT *pInitParams)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint16_t status;
    uint8_t filterConf = 0; // Default DISABLED CAN_FILTER_16BIT
                            // CAN_FILTER_ID_MASK CAN_MSG_RX_FIFO0
    ...
    if (pInitParams->FilterScale == CAN_FILTER_32BIT) {
        ...
        if (brgStat == BRG_NO_ERR) {
            if (pInitParams->FilterMode ==
                CAN_FILTER_ID_MASK) { // Mask0 in FilterMaskHigh+Low
                brgStat =
                    FormatFilter32bitCAN(&pInitParams->Mask[0], &filterMask[0]);
            } else { // CAN_FILTER_ID_LIST, ID1 in FilterMaskHigh+Low
                brgStat =
                    FormatFilter32bitCAN(&pInitParams->Id[1], &filterMask[0]);
            }
        }
        ...
        if (pInitParams->FilterMode == CAN_FILTER_ID_MASK) {
            if (brgStat == BRG_NO_ERR) { // Mask0 in MaskIdHigh
                brgStat =
                    FormatFilter16bitCAN(&pInitParams->Mask[0], &filterMask[2]);
            }
            if (brgStat == BRG_NO_ERR) { // ID1 in FilterIdLow
                brgStat =
                    FormatFilter16bitCAN(&pInitParams->Id[1], &filterId[0]);
            }
            if (brgStat == BRG_NO_ERR) { // Mask1 in MaskIdLow
                brgStat =
                    FormatFilter16bitCAN(&pInitParams->Mask[1], &filterMask[0]);
            }
            ...
            if (brgStat == BRG_NO_ERR) { // ID2 in MaskIdHigh
                brgStat =
                    FormatFilter16bitCAN(&pInitParams->Id[2], &filterMask[2]);
            }
            if (brgStat == BRG_NO_ERR) { // ID3 in MaskIdLow
                brgStat =
                    FormatFilter16bitCAN(&pInitParams->Id[3], &filterMask[0]);
            }
        }
    }
    if (brgStat != BRG_NO_ERR) {
...
/**
 * @ingroup CAN
 * @brief This routine activates the CAN RX messages reception storage at STLink
 * side.\n Brg::StartMsgReceptionCAN() must be called before
 * Brg::GetRxMsgNbCAN() and Brg::GetRxMsgCAN().\n CAN init must have been called
 * before and CAN filter activation is needed to be able to receive CAN
 * messages.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If CAN is not initialized
 * @retval #BRG_CAN_ERR In case of CAN error
 * @retval #BRG_PARAM_ERR If message format is different between host and
 * firmware
 * @retval #BRG_CMD_NOT_SUPPORTED If firmware is too old for this command
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::StartMsgReceptionCAN(void)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint8_t answer[4];
    ...
    brgStat = SendRequestAndAnalyzeStatus(pRq, (uint16_t *)answer);
    if ((answer[2] != CAN_MSG_FORMAT_V1) &&
        (brgStat == BRG_NO_ERR)) { // robustness
        StopMsgReceptionCAN();
        brgStat = BRG_PARAM_ERR;
    }
    if (brgStat != BRG_NO_ERR) {
        LogTrace(
            "CAN Error (%d) in StartMsgReceptionCAN (firmware msg format: %d, "
            "host format: %d)",
 ...
 * @param[out]  pMsgNb  Filled with the number of available RX CAN messages at
 * STLink side.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If CAN is not initialized
 ...
 */
Brg_StatusT
Brg::GetRxMsgNbCAN(uint16_t *pMsgNb)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint8_t answer[8];
...
/**
 * @ingroup CAN
 * @brief This routine allows to get the available CAN messages receieved
 * through the CAN interface and stored by the STLink.\n The number of available
 * messages (MsgNb) MUST be retrieved first with Brg::GetRxMsgNbCAN().
 * @param[out]  pCanMsg pointer on array of at least MsgNb message "header":
 * #Brg_CanRxMsgT. Each message fields are updated as described in
 * #Brg_CanRxMsgT.
 * @param[in]  MsgNb MUST be the value returned by Brg::GetRxMsgNbCAN().
 *                    It may be less than the value returned by
 * Brg::GetRxMsgNbCAN() only if the messages are retrieved with several call to
 * Brg::GetRxMsgCAN().\n pCanMsg must point on a #Brg_CanRxMsgT array with at
 * least MsgNb element.\n
 * @warning     If MsgNb > Brg::GetRxMsgNbCAN(), firmware will return status
 * error (2bytes) instead of requested messages this may break the USB com.
 * @param[out]  pBuffer  Data buffer for read data (filled only for data frame
 * with DLC>0).
 * @param[out]  BufSizeInBytes  Is less or equal to real data buffer size in
 * bytes. It indicates pBuffer available size for read data.\n Note: data size
 * that can be received is less or equal to MsgNb*8 (DLC max =8).\n If
 * BufSizeInBytes is smaller than the sum of all DLC only BufSizeInBytes are
 * copied into pBuffer.
 * @param[out]  pDataSizeInBytes  Filled with the number of data written in
 * pBuffer (in bytes).
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If CAN is not initialized
 * @retval #BRG_CAN_ERR In case of CAN error
 * @retval #BRG_PARAM_ERR If incorrect MsgNb (0 or greater than available
 * messages) or NULL pointer
 * @retval #BRG_CMD_NOT_SUPPORTED If firmware is too old for this command
 ...
 */
Brg_StatusT
Brg::GetRxMsgCAN(Brg_CanRxMsgT *pCanMsg, uint16_t MsgNb, uint8_t *pBuffer,
                 uint16_t BufSizeInBytes, uint16_t *pDataSizeInBytes)
{
    ...
    if ((pCanMsg == NULL) || (pBuffer == NULL) || (pDataSizeInBytes == NULL) ||
        (MsgNb < 1)) {
        return BRG_PARAM_ERR;
    }
    ...
    *pDataSizeInBytes = 0; // Default
    ...
    // Warning if MsgNb is not correct, a 2 bytes error status is received from
    // the FW instead of answerSize bytes, this is a host issue and can lead to
    // USB com err or wrongly interpreted answer
    if (brgStat == BRG_NO_ERR) {
        uint8_t overrunErr;
        ...
        for (int j = 0; j < MsgNb; j++) {
            ...
            pCanMsg[j].ID = (uint32_t)pReadCanMsg[0] |
                            (((uint32_t)pReadCanMsg[1]) << 8) |
                            (((uint32_t)pReadCanMsg[2]) << 16) |
                            (((uint32_t)pReadCanMsg[3]) << 24);
            // byte4 message type
            ...
            if (overrunErr != 0) {
                ...
                if (brgStat == BRG_NO_ERR) {
                    ...
                    firstErrMsgNb = j;
                    LogTrace("CAN Overrun Error in GetRxMsgCAN (first error %d "
                             "at %d/%d msg)",
            ...
            if ((pReadCanMsg[4] & 0x2) == 0) { // byte4 bit1 RTR
                ...
                if (buffDataSize >= pCanMsg[j].DLC) {
                    ...
                    msgDataSize =
                        buffDataSize; // limit copied data to max buffer size
                    if (brgStat == BRG_NO_ERR) {
                        brgStat = BRG_OVERRUN_ERR;
                        LogTrace("CAN Data Error in GetRxMsgCAN: "
                                 "BufSizeInBytes too small "
                                 "(error at %d/%d msg)",
                                 (int)j, (int)MsgNb);
                    }
                }
            ...
            for (int i = 0; i < msgDataSize; i++) {
                pBuffer[buffDataOffset + i] =
                    pReadCanMsg[CAN_READ_MSG_HEADER_SIZE_V1 + i];
            }
            // Point on next message and update the number of remaining data to
            // copy
    ...
    if (brgStat != BRG_NO_ERR) {
        ...
        if (BufSizeInBytes == 4) { // BufSizeInBytes
            LogTrace("CAN R %d msg: 0x%08lx", (int)MsgNb,
...
/**
 * @ingroup CAN
 * @brief This routine allows to send a message on CAN bus through the CAN
 * interface, in the mode initialized by InitCAN().\n
 * @param[in]   pCanMsg Pointer on a message "header" see #Brg_CanTxMsgT
 * description.
 * @param[in]   pBuffer  Pointer to the data buffer (must be at least size
 * length).
 ...
 */
Brg_StatusT
Brg::WriteMsgCAN(const Brg_CanTxMsgT *pCanMsg, const uint8_t *pBuffer,
                 uint8_t SizeInBytes)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    ...
    if ((pCanMsg->DLC > 8) ||
        (SizeInBytes > 8)) { // CAN message DATA FIELD = 8 bytes max
        return BRG_PARAM_ERR;
    }
    ...
    if (brgStat == BRG_NO_ERR) { // pSizeWritten not useful for CAN, pErrorInfo
                                 // currently unused
        brgStat = GetLastReadWriteStatus(NULL, NULL);
    }

    if (brgStat != BRG_NO_ERR) {
        LogTrace("CAN Error (%d) in WriteMsgCAN (%d bytes)", (int)brgStat,
                 (int)SizeInBytes);
    }
    return brgStat;
}
 ...
 * Used internally be classic Read/Write bridge function and need to be called
 * by user in some I2C cases.
 * @warning In #BRG_CMD_BUSY case, this is the only command that can be sent to
 * the STLink.
 *
 * @param[out] pBytesWithoutError If not NULL and in case of error, returns the
 * number of bytes transmitted/received before the error (status !=
 * #BRG_NO_ERR).
 * @param[out] pErrorInfo Currently not significant (=0 if pErrorInfo!=NULL)
 *
 * @return All possible read/write errors
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_BUSY STLink is BUSY state: need to poll later again until
 * BUSY no more returned
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::GetLastReadWriteStatus(uint16_t *pBytesWithoutError, uint32_t *pErrorInfo)
{
    uint16_t answer[BRIDGE_RW_STATUS_LEN_WORD] = {0, 0, 0, 0};
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
...
// -------------------------------- GPIO ----------------------------------- //
/*
 * private: return the gpio configuration field of STLINK_BRIDGE_INIT_GPIO
 * according to init parameter
 */
uint8_t
Brg::GpioConfField(Brg_GpioConfT GpioConfParam)
{
    // return gpioConf: Bit1-0 mode, Bit3-2 speed, Bit5-4 pull, Bit6 output type
    return (uint8_t)(((uint8_t)GpioConfParam.Mode & 0x03) |
                     ((((uint8_t)GpioConfParam.Speed) << 2) & 0x0C) |
                     ((((uint8_t)GpioConfParam.Pull) << 4) & 0x30) |
                     ((((uint8_t)GpioConfParam.OutputType) << 6) & 0x40));
}
/**
 * @ingroup GPIO
 * @brief This routine initializes the given GPIO according to init
 * parameters.\n Refer to STLink firmware upgrade release note (stsw-link007)
 * for supported init configurations.\n
 * @param[in]  pInitParams GPIO initialization parameters see #Brg_GpioInitT and
 * \ref LIMITATION
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_PARAM_ERR If NULL pointer or if ConfigNb is not 1 or
 * #BRG_GPIO_MAX_NB
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT
Brg::InitGPIO(const Brg_GpioInitT *pInitParams)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint16_t status;
    ...
    if (((pInitParams->ConfigNb != 1) &&
         (pInitParams->ConfigNb != BRG_GPIO_MAX_NB)) ||
    ...
    // GPIO 0, 1, 2, 3 config, Bit1-0 mode, Bit3-2 speed, Bit5-4 pull, Bit6
    // output type
 ...
 * @param[in]  GpioMask  GPIO(s) to be read (one or several value of
 * #Brg_GpioMaskT)
 * @param[out] pGpioVal Table of #BRG_GPIO_MAX_NB #Brg_GpioValT filled with gpio
 * read value, pGpioVal[0] for GPIO0, ..., pGpioVal[3] for GPIO3
 * @param[out] pGpioErrorMask  0 if no error, else corresponding GPIO bit is set
 * to 1, error may occur if GPIO has not been initialized previously in
 * Brg::InitGPIO()
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 ...
 */
Brg_StatusT
Brg::ReadGPIO(uint8_t GpioMask, Brg_GpioValT *pGpioVal, uint8_t *pGpioErrorMask)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint8_t answer[8] = {0, 0, 0, 0, 0, 0, 0, 0};

    if ((pGpioVal == NULL) || (pGpioErrorMask == NULL) ||
        ((GpioMask & BRG_GPIO_ALL) == 0)) {
        return BRG_PARAM_ERR;
    }
    ...
    *pGpioErrorMask = answer[2];
    ...
    // Answer byte3 GPIO read value (0 or 1), if GPIO is present in the mask,
 ...
 * @param[in]  GpioMask  GPIO(s) to be set or reset (one or several value of
 * #Brg_GpioMaskT)
 * @param[in]  pGpioVal Table of #BRG_GPIO_MAX_NB #Brg_GpioValT containing value
 * to write, pGpioVal[0] for GPIO0, ..., pGpioVal[3] for GPIO3
 * @param[out] pGpioErrorMask  0 if no error, else corresponding GPIO bit is set
 * to 1, error may occur if GPIO has not been initialized previously in
 * Brg::InitGPIO()
 *
 ...
 */
Brg_StatusT
Brg::SetResetGPIO(uint8_t GpioMask, const Brg_GpioValT *pGpioVal,
                  uint8_t *pGpioErrorMask)
{
    STLink_DeviceRequestT *pRq;
    Brg_StatusT brgStat;
    uint8_t answer[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    ...
    // GPIO mask
    pRq->CDBByte[2] = GpioMask;
    // GPIO set reset mask (1 if  set, 0 if reset), if GPIO is present in the
    // mask set the value to write.
    for (int i = 0; i < BRG_GPIO_MAX_NB; i++) {
        if ((GpioMask & (1 << i)) != 0) {
...
/**
 * @ingroup BRIDGE
 * @brief This routine gets target voltage in V, computed from STLink VREFINT
 * value (typically 1.2V at 25C).
 ...
 */
Brg_StatusT
Brg::GetTargetVoltage(float *pVoltage)
{
    STLinkIf_StatusT ifStatus = STLINKIF_NO_ERR;
 ...
 */
bool
Brg::IsCanSupport(void) const
{
    // Return false if FW is too old for full CAN support
    if (((m_Version.Major_Ver == 3) &&
         (m_Version.Bridge_Ver < FIRMWARE_BRIDGE_MIN_VER_FOR_CAN))) {
        return false;
    } else {
        return true;
    }
 ...
 */
bool
Brg::IsReadNoWaitI2CSupport(void) const
{
    // Return false if FW is too old for READ_NO_WAIT_I2C command support
    if (((m_Version.Major_Ver == 3) &&
         (m_Version.Bridge_Ver <
          FIRMWARE_BRIDGE_MIN_VER_FOR_READ_NO_WAIT_I2C))) {
        return false;
    } else {
        return true;
    }
